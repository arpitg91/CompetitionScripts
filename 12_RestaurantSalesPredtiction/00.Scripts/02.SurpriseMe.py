...
data['tes']['visit_date'] = pd.to_datetime(data['tes']['id'].map(lambda x: str(x).split('_')[2]))
...
tmp = data['tra'].groupby(['air_store_id','dow'], as_index=False)['visitors'].agg({np.min,np.mean,np.median,np.max,np.size})
