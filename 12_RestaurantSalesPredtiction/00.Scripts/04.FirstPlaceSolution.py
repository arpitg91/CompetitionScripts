...
def get_reserve_feat(label,key):
    ...
    air_reserve_temp = air_reserve[(air_reserve.visit_date >= key[0]) &             # key[0] ?'2017-04-23'
                                   (air_reserve.visit_date < label_end_date) &      # label_end_date ?'2017-05-31'
...
store=pd.HDFStore('../01.RawData/FirstPlaceSol.h5')
store['train_feat']=train_feat
store['test_feat']=test_feat
store.close()
output = open('../01.RawData/FirstPlaceSol.pkl', 'wb')
pickle.dump(mydict, gbm)
output.close()                  